...
impl Error {
    #[cfg(feature = "parse")]
    pub(crate) fn new(input: alloc::sync::Arc<str>, error: env_toml_parser::ParseError) -> Self {
        ...
        if let Some(expected) = error.expected() {
            ...
            if expected.is_empty() {
                ...
                for (i, expected) in expected.iter().enumerate() {
                    ...
                    match expected {
                        env_toml_parser::Expected::Literal(desc) => {
                            message.push_str(&render_literal(desc));
                        }
                        env_toml_parser::Expected::Description(desc) => message.push_str(desc),
...
impl<'i, S: Default> TomlSink<'i, S> {
    pub(crate) fn new(source: env_toml_parser::Source<'i>) -> Self {
        Self {
            source,
            input: None,
...
#[cfg(feature = "parse")]
impl<'i> env_toml_parser::ErrorSink for TomlSink<'i, Option<Error>> {
    fn report_error(&mut self, error: env_toml_parser::ParseError) {
...
#[cfg(feature = "parse")]
impl<'i> env_toml_parser::ErrorSink for TomlSink<'i, Vec<Error>> {
    fn report_error(&mut self, error: env_toml_parser::ParseError) {
        let input = self
            .input
            .get_or_insert_with(|| alloc::sync::Arc::from(self.source.input()));
