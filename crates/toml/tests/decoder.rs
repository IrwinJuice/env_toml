...
impl toml_test_harness::Decoder for Decoder {
    ...
    fn decode(
        ...
        let (table, errors) = env_toml::de::DeTable::parse_recoverable(data);
        if !errors.is_empty() {
        ...
        let document =
            env_toml::Table::deserialize(env_toml::de::Deserializer::from(table)).map_err(|mut err| {
                err.set_input(Some(data));
                toml_test_harness::Error::new(err)
            })?;
        let value = env_toml::Value::Table(document);
        value_to_decoded(&value)
    }
}

fn value_to_decoded(
    value: &env_toml::Value,
) -> Result<toml_test_harness::DecodedValue, toml_test_harness::Error> {
    match value {
        env_toml::Value::Integer(v) => Ok(toml_test_harness::DecodedValue::Scalar(
            toml_test_harness::DecodedScalar::from(*v),
        )),
        env_toml::Value::String(v) => Ok(toml_test_harness::DecodedValue::Scalar(
            toml_test_harness::DecodedScalar::from(v),
        )),
        env_toml::Value::Float(v) => Ok(toml_test_harness::DecodedValue::Scalar(
            toml_test_harness::DecodedScalar::from(*v),
        )),
        &env_toml::Value::Datetime(mut v) => {
        ...
        env_toml::Value::Boolean(v) => Ok(toml_test_harness::DecodedValue::Scalar(
            toml_test_harness::DecodedScalar::from(*v),
        )),
        env_toml::Value::Array(v) => {
            let v: Result<_, toml_test_harness::Error> = v.iter().map(value_to_decoded).collect();
            Ok(toml_test_harness::DecodedValue::Array(v?))
        }
        env_toml::Value::Table(v) => table_to_decoded(v),
    }
}

fn table_to_decoded(
    value: &env_toml::value::Table,
) -> Result<toml_test_harness::DecodedValue, toml_test_harness::Error> {
